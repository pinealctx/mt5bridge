...
This project uses example configuration files (`.example.json`) as templates. You need to create actual configuration files based on your environment.

### Configuration Files List

| File                       | Description            | Required     |
| :------------------------- | :--------------------- | :----------- |
| `appsettings.example.json` | App config example     | Template     |
| `appsettings.json`         | Actual app config      | **Required** |
| `nlog.example.json`        | Logging config example | Template     |
| `nlog.json`                | Actual logging config  | Optional     |

> **Note:** `appsettings.json` and `nlog.json` are added to `.gitignore` and will not be committed to version control.

### Quick Setup

#### 1. Create App Configuration (Required)
```powershell
# Windows PowerShell
Copy-Item appsettings.example.json appsettings.json
```
Then edit `appsettings.json` with your MT5 connection info.

#### 2. Create Logging Configuration (Optional)
```powershell
# Windows PowerShell
Copy-Item nlog.example.json nlog.json
```
If `nlog.json` is not created, NLog will use the default configuration.

### Security Recommendations
1. ✅ **Do not commit** `appsettings.json` and `nlog.json` to version control.
2. ✅ Use strong passwords and change them regularly.
3. ✅ Use a dedicated Manager account for production.
4. ✅ Regularly check log files and clean up sensitive information.
